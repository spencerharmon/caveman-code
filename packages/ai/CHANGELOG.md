...
- Fixed empty Claude extended-thinking text on Anthropic-relay providers for Opus 4.7+ (e.g. `github-copilot/claude-opus-4.7`, `claude-opus-4.7-1m-internal`, `claude-opus-4.8`, and direct `anthropic/claude-opus-4-7`): Anthropic flipped the API default for the adaptive `thinking.display` field to `"omitted"` for these models, which caused the relay to emit thinking content blocks whose `thinking` text was empty (only `thinkingSignature` was populated, so no `thinking_delta` deltas were streamed). The request builder now sends `thinking: { type: "adaptive", display: "summarized" }` for every Opus 4.7+ id on every relay so cleartext reasoning streams just like Opus 4.6 / Sonnet 4.6. Redacted thinking blocks still surface the `[Reasoning redacted]` placeholder unchanged. Mirrors opencode's `anthropicOpus47OrLater` handling.
